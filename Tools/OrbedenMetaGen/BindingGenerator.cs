...
internal sealed class BindingGenerator(BindingModel model, BindingTypes types)
{
    ...
    private string GenerateCpp(string sourceRoot)
    {
        ...
        foreach (string file in model.ObjectTypes.Concat(types.Values.Values.Where(value => value.Declaration != null).Select(value => value.Declaration!)).Where(type => !model.ImportedNames.Contains(type.QualifiedName)).Select(type => type.File).Distinct().Order(StringComparer.Ordinal))
    ...
    private string GenerateCSharp()
    {
        ...
        {
            if (model.ImportedNames.Contains(value.Declaration!.QualifiedName)) continue;
