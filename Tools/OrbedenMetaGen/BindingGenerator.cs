...
internal sealed class BindingGenerator(BindingModel model, BindingTypes types, bool gameModule = false)
{
        ...
        switch (value.Kind)
        {
            ...
            case "record":
                foreach (CppMember field in model.ExportedMembers(value.Declaration!).Where(member => !member.IsMethod && !member.IsStatic))
                    output.AppendLine($"    Write_{types.Resolve(field.Type, value.Declaration!).Key}(writer, {(field.FixedArray ? $"std::to_array(value.{field.Name})" : $"value.{field.Name}")});");
                break;
        ...
        switch (value.Kind)
        {
            ...
            case "array":
                if (value.Cpp.StartsWith("std::array<"))
                    output.AppendLine($"    int32 count = reader.Count();\n    {value.Cpp} value{{}};\n    if (count != static_cast<int32>(value.size())) throw NativeBindingError(NativeBindingStatus::InvalidArgument, \"Fixed array size mismatch\");\n    for (int32 index = 0; index < count; ++index) value[index] = Read_{value.Element!.Key}(reader);\n    return value;");
                else output.AppendLine($"    int32 count = reader.Count();\n    {value.Cpp} value; value.reserve(count);\n    for (int32 index = 0; index < count; ++index) value.push_back(Read_{value.Element!.Key}(reader));\n    return value;");
                break;
            case "record":
                output.AppendLine($"    {value.Cpp} value{{}};");
                foreach (CppMember field in model.ExportedMembers(value.Declaration!).Where(member => !member.IsMethod && !member.IsStatic))
                    if (field.FixedArray)
                        output.AppendLine($"    auto {field.Name} = Read_{types.Resolve(field.Type, value.Declaration!).Key}(reader);\n    std::copy({field.Name}.begin(), {field.Name}.end(), std::begin(value.{field.Name}));");
                    else output.AppendLine($"    value.{field.Name} = Read_{types.Resolve(field.Type, value.Declaration!).Key}(reader);");
    ...
    private void EmitCppCall(StringBuilder output, CppType type, BindingCall call, int index)
    {
        ...
        if (call.Member.FixedArray && call.Operation == "get") expression = $"std::to_array({expression})";
        if (call.Member.FixedArray && call.Operation == "set" && call.Member.Setter.Length == 0)
            expression = $"std::copy({arguments[0]}.begin(), {arguments[0]}.end(), std::begin({target}{call.Name}))";
    ...
    {
        //绑定命名空间随项目名变化（<项目名>.Native），但内容根里的脚本要能跨项目复用，
        //不能写死项目名。给出 Native 别名，脚本写 Native.X 就能引用本程序集的绑定类型。
        //global using 必须排在普通 using 之前，所以别名写在文件头。
        StringBuilder output = new("// <auto-generated />\n#nullable enable\n");
        if (gameModule) output.AppendLine($"global using Native = {model.ManagedNamespace};");
