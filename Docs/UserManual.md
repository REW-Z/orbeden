...
Inspector 按当前选择切换内容，同一块面板服务三类检视：

- **场景对象**：选 Ens 后编辑名称与全部组件；多选可批量编辑同名字段，改动可撤销重做。
- **引擎内部资源**：在 Project 中选 `.orbmat` 材质资产，可直接编辑它的 Shader、绘制队列、颜色槽、浮点槽和纹理槽，保存后写回 `.orbmat` 文本源。`.orbo`、`.orbshader`、`.glsl` 只读展示其内部对象。
- **外部原始资源**：在 Project 中选图片或模型文件，可编辑它的**导入设置**，并查看这次导入产生了哪些内部对象。图片能设颜色空间；模型能设缩放倍率与坐标系上轴（Blender、CAD 导出的 Z-up 转成引擎的 Y-up）。

场景对象优先于资源：两者同时有选择时回到组件检查。

**`.world` 是场景而不是导入资源**：双击或 `Open` 会由编辑器装载它，Ens 与组件通过场景视图和 Ens View 编辑。在 Project 中选中它，Inspector 给出的是**世界级设置**——设为启动场景，以及环境设置（天空盒、环境光）。环境设置不在任何 Ens 上，Ens View 里找不到它。
